...
dw_scrollObj.prototype.setBarSize = function() {
  ...
  if (this.vBarId) {
	bar = document.getElementById(this.vBarId);
    if(!this.bar) setTimeout(";", 300);
    bar.style.height = (lyr.offsetHeight > wn.offsetHeight)? bar.trkHt / ( lyr.offsetHeight / wn.offsetHeight ) + "px": bar.trkHt - 2*bar.minY + "px";
	bar.maxY = bar.trkHt - bar.offsetHeight - bar.minY;
	//alert('final maxh: ' + this.maxheight);
	if( lyr.offsetHeight < this.maxheight) {
		document.getElementById(this.scrollLayer).style.visibility = 'hidden';
	} else if( lyr.offsetHeight >= this.maxheight) { 
		document.getElementById(this.scrollLayer).style.visibility = 'visible';
	}
}
  if (this.hBarId) {
    bar = document.getElementById(this.hBarId);
