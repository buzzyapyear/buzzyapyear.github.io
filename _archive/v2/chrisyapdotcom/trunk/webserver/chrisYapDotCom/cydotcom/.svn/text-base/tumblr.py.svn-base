...
#
# Modifications by Chris Yap to Api.read to add more parameters to the 
# request.
#
...
class Api:
	...
	def read(self, id=None, tags='', numberToDisplay=6, start=0,max=2**31-1,type=None): 
		if id:
			url = "http://%s.tumblr.com/api/read/json?tagged=%s&num=%s" % (id, tags, numberToDisplay)
			#url = "http://buzzyapyear.tumblr.com/api/read/json?tagged=portfolioSite&num=6"
			response = urlopen(url)
			page = response.read()
			m = re.match("^.*?({.*}).*$", page,re.DOTALL | re.MULTILINE | re.UNICODE)
