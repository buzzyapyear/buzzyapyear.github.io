...
import os
import re
from cydotcom import contexts
from cydotcom.settings import BRANCH_MOBILE
...
def render_to_response(request, template_name, data=None, mimetype=None):
    ...
    return response
"""
# this function has been modified to automatically switch template path
# for mobile user agents
def render_to_response(request, template_name, data=None, mimetype=None):

    if BRANCH_MOBILE:
        mobileSniff = contexts.mobileSniff(request)
        if mobileSniff['isMobile']:
            # ignore standalone urls
            p = re.compile('standalone', re.IGNORECASE)
            if not p.search(template_name):
